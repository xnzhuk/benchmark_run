...
def process_data():
    ...
    if os.path.exists("Cinebenchr20\\single_result.txt"):
        # 读取single_result.txt文件内容
        with open("Cinebenchr20\\single_result.txt", "r") as file:
            content = file.read()
        ...
        with open("Cinebenchr20\\single_output.txt", "w") as output_file:
            output_file.write("\n".join(numbers))
        ...
        print("Cinebenchr20\\single_result.txt文件不存在")

    # 判断multi_result.txt文件是否存在
    if os.path.exists("Cinebenchr20\\multi_result.txt"):
        # 读取multi_result.txt文件内容
        with open("Cinebenchr20\\multi_result.txt", "r") as file:
            content = file.read()
        ...
        with open("Cinebenchr20\\multi_output.txt", "w") as output_file:
            output_file.write("\n".join(numbers))
...
def option_one():
    ...
    for _ in range(num_rounds):
        command = 'start /b /wait cmd.exe /C ".\\Cinebenchr20\\Cinebench.exe g_CinebenchCpuXTest=false g_CinebenchCpu1Test=true g_CinebenchMinimumTestDuration=1 >> .\\Cinebenchr20\\single_result.txt"'
...
def option_two():
    ...
    for _ in range(num_rounds):
        command = 'start /b /wait cmd.exe /C ".\\Cinebenchr20\\Cinebench.exe g_CinebenchCpuXTest=true g_CinebenchCpu1Test=false g_CinebenchMinimumTestDuration=1 >> .\\Cinebenchr20\\multi_result.txt"'
        subprocess.call(command, shell=True)
        print("Multi被执行了")
        time.sleep(interval)
    process_data()
