...
class Application(tk.Tk):
    def __init__(self):
        ...
        tk.Label(self, text="选择测试类型").pack()
        self.selected_option = tk.StringVar()
        self.selected_option.set("1")
        tk.Radiobutton(self, text="Firestrike", variable=self.selected_option, value="1").pack(anchor=tk.W)
        tk.Radiobutton(self, text="Nightraid", variable=self.selected_option, value="2").pack(anchor=tk.W)
        tk.Radiobutton(self, text="Timespy", variable=self.selected_option, value="3").pack(anchor=tk.W)

        tk.Label(self, text="输入测试圈数").pack()
        self.rounds_entry = tk.Entry(self)
        self.rounds_entry.pack()

        tk.Label(self, text="输入每圈间隔时间（秒）").pack()
        self.interval_entry = tk.Entry(self)
        self.interval_entry.pack()

        self.run_button = tk.Button(self, text="运行测试", command=self.run_test)
        self.run_button.pack(pady=20)

    def run_test(self):
        test_type = int(self.selected_option.get())
        rounds = int(self.rounds_entry.get())
        interval = int(self.interval_entry.get())

        if test_type == 1:
            option_one(rounds, interval)
        elif test_type == 2:
            option_two(rounds, interval)
        elif test_type == 3:
            option_three(rounds, interval)
...
# 判断firestrike_input.xml文件是否存在，解析XML文件并写入数据
# （你的firestrike_data函数的代码）
...
# 判断nightraid_input.xml文件是否存在，解析XML文件并写入数据
# （你的nightraid_data函数的代码）
...
# 判断timespy_input.xml文件是否存在，解析XML文件并写入数据
# （你的timespy_data函数的代码）
